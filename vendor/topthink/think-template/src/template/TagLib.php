    ...
    public function parseTag(string &$content, string $lib = ''): void
    {
        ...
        if (!empty($tags[1])) {
            ...
            if ($nodes) {
                ...
                // 自定义loop标签
                if($name=='loop'){
                    $loop_str=preg_replace( "/\s(?=\s)/","\\1", trim($attrs['expression']) );
                    $loop_array=explode(' ',$loop_str);
                    if(count($loop_array)==2){
                        $loop_str=$loop_array[0].' as '.$loop_array[1];
                    }elseif(count($loop_array)==3){
                        $loop_str=$loop_array[0].' as '.$loop_array[1].' => '.$loop_array[2];
                    }else{

                    }
                    $attrs['expression']=$loop_str;
                    //禁止自动补上$
                    $attrs['sunphp_no_build_var']=true;
                    $method = 'tagForeach';
                }

                    // 读取标签库中对应的标签内容 replace[0]用来替换标签头，replace[1]用来替换标签尾

                    $tag_result=$this->$method($attrs, $break);
                    $replace = explode($break, $tag_result);
            ...
            $content = preg_replace_callback($regex, function ($matches) use (&$tags, &$lib) {
                ...
                if(in_array($matches[1],$mytag_list)){
                  ...
                  if(!empty($mytag_matches)){
                    // 添加可能缺失的分号
                    $mytag_content=rtrim($mytag_matches[1]);
                    if(substr($mytag_content,-1)!=';'){
                        $mytag_content.=';';
                    }
                    if($matches[1]=='php '){
                        $method = 'tagPhp';
                    }
                    return $this->$method($attrs, $mytag_content);

                  }else{
                    return $this->$method($attrs, '');
                  }
                }else if($matches[1]=='else if'){
                    // 排序是从后往前
                    $method='tagElseif';
                    return $this->$method($attrs, '');
                }else if($matches[1]=='template'){
                     // 排序是从后往前
                     return $this->$method($attrs, preg_replace('/\'|\"/','',trim($attrs['expression'])));
                }else{
                    // 默认返回
                    return $this->$method($attrs, '');
                }
