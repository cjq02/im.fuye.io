...
class Cx extends Taglib
{
    ...
    protected $tags = [
        ...
        // 自定义标签loop
        'loop'    => ['attr' => 'name,id,item,key,offset,length,mod', 'expression' => true],

        // 自定义标签template
        'template'=>['attr' => '', 'close' => 0,'expression' => true],

        'if'         => ['attr' => 'condition', 'expression' => true],
        'elseif'     => ['attr' => 'condition', 'close' => 0, 'expression' => true],

        // 自定义标签else if
        'else if'     => ['attr' => 'condition', 'close' => 0, 'expression' => true],
    ...
    public function tagTemplate(array $tag, string $content): string
    {
        return '<?php echo sunphp_addons_template(\''.$content.'\')?>';
    }
    ...
    public function tagForeach(array $tag, string $content): string
    {
        ...
        if (!empty($tag['expression'])) {
            ...
            //自定义，禁止补上$
            if(empty($tag['sunphp_no_build_var'])){
    ...
    public function tagIf(array $tag, string $content): string
    {
        ...
        if(!empty($_W['addons_index'])&&in_array($_W['addons_index'],['app','web'])){
            ...
            if(preg_match_all('/(\$[0-9a-z_A-Z\'\"\[\]]*)/',$condition,$matches)){
                $m_str='';
